...
from lib.dataset.auto_augment import rand_augment_transform, augment_and_mix_transform, auto_augment_transform
from lib.dataset.utils import RandomErasing, _pil_interp, RandomResizedCropAndInterpolation, ToNumpy
...
def transforms_imagenet_train(
    ...
    if elastic_img_size != 224:
        img_size = [elastic_img_size, img_size]

    if isinstance(img_size, list):
        resize_transform_class = MyRandomResizedCrop
        MyRandomResizedCrop.IMAGE_SIZE_LIST = img_size.copy()
        MyRandomResizedCrop.ACTIVE_SIZE = max(img_size)
        ...
        resize_transform_class = RandomResizedCropAndInterpolation
        primary_tfl = [
            resize_transform_class(
    ...
    if tf_preprocessing and use_prefetcher:
        ...
        if is_training:
            transform = transforms_imagenet_train(
                ...
                mean=mean,
                std=std,
                elastic_img_size=elastic_img_size,
                re_prob=re_prob,
                re_mode=re_mode,
                re_count=re_count,
